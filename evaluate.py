...
SQL:"""
    ...
    def evaluate(
        ...
        for example in tqdm(test_data, desc="Evaluating"):
            ...
            # Capture debug info (if any)
            debug_info = (
                reward_dict.get("debug", {}) if isinstance(reward_dict, dict) else {}
            )
        ...
        metrics = {
            ...
            "precision": precision_score(
                ground_truth, predictions_binary, zero_division=0
            ),
...
def main(args):
    ...
    metrics = evaluator.evaluate(
        test_data=test_data, db_root=args.db_root, output_file=args.output_file
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to model or adapter"
    )
    parser.add_argument(
        "--base_model", type=str, required=True, help="Base model name/path"
    )
    parser.add_argument(
        "--test_data", type=str, required=True, help="Path to test data JSON"
    )
    parser.add_argument(
        "--db_root", type=str, required=True, help="Database root directory"
    )
